/*
 *  kernel_cfg.c
 *  Wed Feb 15 07:34:15 2017
 *  SG Version 2.00
 *  sg.exe ./BRO_spam_client.oil -os=ECC2 -I/home/ubuntu/NXT/nxtOSEK/ecrobot/../toppers_osek/sg/impl_oil -template=/home/ubuntu/NXT/nxtOSEK/ecrobot/../toppers_osek/sg/lego_nxt.sgt
 */
...
const TaskType ConfigTask = 0;
const TaskType SimulTask = 1;
const TaskType DisplayTask = 2;

extern void TASKNAME( ConfigTask )( void );
extern void TASKNAME( SimulTask )( void );
extern void TASKNAME( DisplayTask )( void );

static __STK_UNIT _stack_ConfigTask[__TCOUNT_STK_UNIT(512)];
static __STK_UNIT _stack_SimulTask[__TCOUNT_STK_UNIT(4096)];
static __STK_UNIT _stack_DisplayTask[__TCOUNT_STK_UNIT(512)];

const Priority tinib_inipri[TNUM_TASK] = { TPRI_MINTASK + 1, TPRI_MINTASK + 4, TPRI_MINTASK + 2, };
const Priority tinib_exepri[TNUM_TASK] = { TPRI_MINTASK + 1, TPRI_MINTASK + 4, TPRI_MINTASK + 2, };
const UINT8 tinib_maxact[TNUM_TASK] = { (1) - 1, (1) - 1, (1) - 1, };
const AppModeType tinib_autoact[TNUM_TASK] = { 0x00000001, 0x00000000, 0x00000000, };
const FP tinib_task[TNUM_TASK] = { TASKNAME( ConfigTask ), TASKNAME( SimulTask ), TASKNAME( DisplayTask ), };
const __STK_UNIT tinib_stk[TNUM_TASK] = { (__STK_UNIT)_stack_ConfigTask, (__STK_UNIT)_stack_SimulTask, (__STK_UNIT)_stack_DisplayTask, };
const UINT16 tinib_stksz[TNUM_TASK] = { 512, 4096, 512, };
...
const AlarmType SimulTaskAlarm = 0;
const AlarmType LcdUpdateAlarm = 1;

DeclareTask(SimulTask);
static void _activate_alarm_SimulTaskAlarm( void );
static void _activate_alarm_SimulTaskAlarm( void )
{ (void)ActivateTask( SimulTask ); }

DeclareTask(DisplayTask);
static void _activate_alarm_LcdUpdateAlarm( void );
static void _activate_alarm_LcdUpdateAlarm( void )
{ (void)ActivateTask( DisplayTask ); }

const CounterType alminib_cntid[TNUM_ALARM] = { 0, 0, };
const FP alminib_cback[TNUM_ALARM] = { _activate_alarm_SimulTaskAlarm, _activate_alarm_LcdUpdateAlarm, };
const AppModeType alminib_autosta[TNUM_ALARM] = { 0x00000000, 0x00000001, };
const TickType alminib_almval[TNUM_ALARM] = { 0, 1, };
const TickType alminib_cycle[TNUM_ALARM] = { 0, 100, };
...
const ResourceType Lcd = 1;
const ResourceType SimConfigGlobal = 2;

const Priority resinib_ceilpri[TNUM_RESOURCE] = { TPRI_SCHEDULER, TPRI_MINTASK + 2, TPRI_MINTASK + 2, };
...
const EventMaskType SimTermEvt = (1UL << 0);
